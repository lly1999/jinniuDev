<template>
    <el-container>
        <el-header style="font-size:25px;padding:5px ;">
            <h5 class="card-title" style="font-size:30px;padding:5px ;">红花堰垃圾站数据统计</h5>
        </el-header>
        <el-main>
...
onBeforeMount(() => {
    ...
    getQuery('红花堰', 'transporter', today, tomorrow, 1, 10000).then(function (resp) {
        total.value = 0;
        for (let i = 0; i < resp.length; i++) {
            total.value = resp[i].netWeight + total.value
        }
    ...
    getQuery('红花堰', 'transporter', today, tomorrow, 1, 10).then(function (resp) {
